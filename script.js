...
function startRain(intensity = 1) {
    ...
    function refreshZones() {
        ...
        document.querySelectorAll(splashSelectors).forEach(el => {
            ...
            if (r.width > 0 && r.height > 0) {
                splashZones.push({ left: r.left, right: r.right, top: r.top });
            }
        });
    }
    ...
    for (let i = 0; i < frontCount; i++) {
        fxFrontParticles.push({
            x: Math.random() * fxCanvas.width,
            y: Math.random() * fxCanvas.height * -0.3,
            len: 20 + Math.random() * 26,
            speed: 10 + Math.random() * 10,
            opacity: 0.08 + Math.random() * 0.15,
            drift: -2 - Math.random() * 2.5,
    ...
    function frame() {
        ...
        for (const p of fxParticles) {
            ...
            fxCtx.strokeStyle = `rgba(174,194,224,${p.opacity})`;
            fxCtx.lineWidth = 1.3;
            fxCtx.stroke();
            p.y += p.speed;
            p.x += p.drift * 0.3;

            const bottomY = p.y + p.len;
            let splashed = false;

            for (const z of splashZones) {
                if (p.x >= z.left && p.x <= z.right && bottomY >= z.top && p.y < z.top + 10) {
                    spawnSplash(p.x, z.top);
                    splashed = true;
                    break;
                }
            }

            if (!splashed && p.y > fxCanvas.height) {
                if (Math.random() < 0.25) spawnSplash(p.x, fxCanvas.height);
            }

            if (splashed || p.y > fxCanvas.height) {
                p.y = Math.random() * -150 - 20;
                p.x = Math.random() * fxCanvas.width;
            }
        }

        for (const p of fxFrontParticles) {
            fxFrontCtx.beginPath();
            fxFrontCtx.moveTo(p.x, p.y);
            fxFrontCtx.lineTo(p.x + p.drift * 0.6, p.y + p.len);
            fxFrontCtx.strokeStyle = `rgba(174,194,224,${p.opacity})`;
            fxFrontCtx.lineWidth = p.width;
            fxFrontCtx.stroke();
            p.y += p.speed;
            p.x += p.drift * 0.3;
        ...
        for (let i = fxSplashes.length - 1; i >= 0; i--) {
            ...
            fxCtx.beginPath();
            fxCtx.arc(s.x, s.y, s.r * s.life, 0, Math.PI * 2);
            fxCtx.fillStyle = `rgba(174,210,240,${s.life * 0.6})`;
            fxCtx.fill();
        }
...
function startSnow(intensity = 1) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        fxParticles.push({
            ...
            wobbleAmp: 20 + depth * 40,
            opacity: 0.2 + depth * 0.5,
            rotation: Math.random() * Math.PI * 2,
            rotSpeed: (Math.random() - 0.5) * 0.02,
            arms: Math.random() > 0.4 ? 6 : 4,
            depth,
        });
    }
    ...
    function drawSnowflake(x, y, r, rotation, arms, opacity) {
        ...
        if (r < 2.5) {
            ...
            fxCtx.beginPath();
            fxCtx.arc(0, 0, r * 1.5, 0, Math.PI * 2);
            fxCtx.fillStyle = grad;
            fxCtx.fill();
        } else {
            fxCtx.strokeStyle = 'rgba(255,255,255,0.9)';
            fxCtx.lineWidth = r * 0.2;
            fxCtx.lineCap = 'round';
            for (let i = 0; i < arms; i++) {
                ...
                const ba2 = a - 0.5;
                fxCtx.beginPath();
                fxCtx.moveTo(cos * bPos, sin * bPos);
                fxCtx.lineTo(cos * bPos + Math.cos(ba1) * bLen, sin * bPos + Math.sin(ba1) * bLen);
                fxCtx.stroke();
                fxCtx.beginPath();
                fxCtx.moveTo(cos * bPos, sin * bPos);
                fxCtx.lineTo(cos * bPos + Math.cos(ba2) * bLen, sin * bPos + Math.sin(ba2) * bLen);
                fxCtx.stroke();
            }
            ...
            fxCtx.beginPath();
            fxCtx.arc(0, 0, r * 0.5, 0, Math.PI * 2);
            fxCtx.fillStyle = cg;
            fxCtx.fill();
        }
    ...
    function frame() {
        ...
        for (const p of fxParticles) {
            ...
            drawSnowflake(drawX, p.y, p.r, p.rotation, p.arms, p.opacity);
            p.y += p.speed;
            p.x += Math.sin(p.wobble) * 0.3;
            if (p.y > fxCanvas.height + p.r * 2) {
                p.y = -p.r * 2;
                p.x = Math.random() * fxCanvas.width;
            }
        }
...
function startSunRays() {
    ...
    function frame() {
        ...
        fxCtx.fillStyle = midGrad;
        fxCtx.beginPath();
        fxCtx.arc(cx, cy, midR, 0, Math.PI * 2);
        fxCtx.fill();
        ...
        fxCtx.beginPath();
        fxCtx.arc(cx, cy, coreR, 0, Math.PI * 2);
        fxCtx.fill();
        ...
        for (let i = 0; i < rays; i++) {
            ...
            const lenBase = 400 + Math.sin(pulse * 1.2 + i * 0.8) * 140;
            const len = lenBase * breathe;
            const width = 0.10 + Math.sin(pulse + i) * 0.025;

            const grad = fxCtx.createLinearGradient(cx, cy, cx + Math.cos(a) * len, cy + Math.sin(a) * len);
            grad.addColorStop(0, 'rgba(255,220,90,0.14)');
            grad.addColorStop(0.3, 'rgba(255,200,60,0.06)');
            grad.addColorStop(1, 'transparent');

            fxCtx.beginPath();
            fxCtx.moveTo(cx, cy);
            fxCtx.lineTo(cx + Math.cos(a - width) * len, cy + Math.sin(a - width) * len);
            fxCtx.lineTo(cx + Math.cos(a + width) * len, cy + Math.sin(a + width) * len);
            fxCtx.closePath();
            fxCtx.fillStyle = grad;
            fxCtx.fill();
        ...
        for (let i = 0; i < rays; i++) {
            ...
            const grad = fxCtx.createLinearGradient(cx, cy, cx + Math.cos(a) * len, cy + Math.sin(a) * len);
            grad.addColorStop(0, 'rgba(255,210,80,0.08)');
            grad.addColorStop(1, 'transparent');
...
function startStars() {
    ...
    for (let i = 0; i < 60; i++) {
        stars.push({
            x: Math.random() * fxCanvas.width,
            y: Math.random() * fxCanvas.height * 0.7,
    ...
    function frame() {
        ...
        for (const s of stars) {
            s.twinkle += s.speed;
            const op = 0.3 + Math.sin(s.twinkle) * 0.3;
            fxCtx.beginPath();
...
function applyWeatherEffect(code, isDay) {
    if ([71,73,75,77,85,86].includes(code)) { startSnow(code >= 75 ? 1.5 : 1); return; }
    if ([95,96,99].includes(code)) { startThunderstorm(); return; }
    if ([51,53,55,56,57,61,63,65,66,67,80,81,82].includes(code)) {
        const heavy = [55,57,65,67,82].includes(code) ? 1.5 : 1;
        startRain(heavy); return;
    }
    if ([45,48].includes(code)) { startFog(); return; }
    if ([0,1].includes(code)) {
        if (isDay) { startSunRays(); return; }
        else { startStars(); return; }
    }
    if (!isDay) { startStars(); return; }
    clearEffects();
}
...
function getWeatherIcon(code, isDay) {
    const map = {
        0: isDay ? 'clear-day' : 'clear-night', 1: isDay ? 'partly-cloudy-day' : 'partly-cloudy-night',
        2: isDay ? 'overcast-day' : 'overcast-night', 3: 'overcast',
        ...
        85: isDay ? 'partly-cloudy-day-snow' : 'partly-cloudy-night-snow',
        86: 'overcast-snow',
...
function setUnit(u) {
    ...
    if (cachedWeatherData) renderWeather();
}

function formatLocation(full) {
    const parts = full.split(', ');
    if (full.length <= 40 || parts.length <= 3) return full;
...
function showState(state) {
    document.getElementById('welcome-state').style.display = state === 'welcome' ? 'block' : 'none';
    ...
    if (state === 'weather') { wc.style.display = ''; wc.classList.add('visible'); }
    else { wc.classList.remove('visible'); wc.style.display = 'none'; }
    if (state !== 'weather') clearEffects();
}
function showError(msg) {
    const el = document.getElementById('error-notification');
    el.textContent = msg; el.style.display = 'block';
}
function hideError() { document.getElementById('error-notification').style.display = 'none'; }
...
async function fetchWeather() {
    ...
    showState('loading');
    try {
        const r = await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(city)}&format=json`, {
            headers: { 'User-Agent': 'Weatherly/2.0' }
        });
        const d = await r.json();
        if (!d?.length) { showError("Location not found."); showState('welcome'); return; }
        await loadWeather(parseFloat(d[0].lat), parseFloat(d[0].lon), d[0].display_name);
    } catch(e) { console.error(e); showError("Failed to fetch location."); showState('welcome'); }
}
...
async function fetchGeoWeather() {
    ...
    navigator.geolocation.getCurrentPosition(async (pos) => {
        try {
            ...
            const d = await r.json();
            await loadWeather(pos.coords.latitude, pos.coords.longitude, d.display_name || 'Your Location');
        } catch(e) { console.error(e); showError("Error getting weather."); showState('welcome'); }
...
async function loadWeather(lat, lon, displayName) {
    try {
        ...
        cachedCoordData = { displayName };
        cachedTimezone = data.timezone;
        renderWeather();
        showState('weather');
    } catch(e) { console.error(e); showError("Error fetching weather data."); showState('welcome'); }
}
...
function renderWeather() {
    ...
    document.body.style.background = isDay
        ? 'linear-gradient(160deg, #1a2235 0%, #1e1a0e 50%, #0f1923 100%)'
        : 'linear-gradient(160deg, #080e18 0%, #0f1923 100%)';

    applyWeatherEffect(current.weathercode, isDay);

    document.getElementById('w-location').textContent = formatLocation(cachedCoordData.displayName);
    document.getElementById('w-location').title = cachedCoordData.displayName;
    ...
    const timeStr = new Date().toLocaleTimeString('en-US', timeOpts);
    document.getElementById('w-datetime').textContent = `${dateStr} • ${timeStr}`;

    document.getElementById('w-icon').innerHTML = getWeatherIcon(current.weathercode, isDay);
    document.getElementById('w-temp').innerHTML = `${toUnit(current.temperature)}<span class="unit">${unitSymbol()}</span>`;
    ...
    document.getElementById('w-feels').textContent = `Feels like ${toUnit(feelsLike)}${unitSymbol()}`;
    ...
    const wind = speedUnit(current.windspeed);
    ...
    if (nowMin < sunriseMin) sunProgress = 0;
    if (nowMin > sunsetMin) sunProgress = 100;
    document.getElementById('w-sun-divider').style.setProperty('--sun-progress', `${sunProgress}%`);

    const dlMin = sunsetMin - sunriseMin;
    const dlH = Math.floor(dlMin / 60);
    const dlM = dlMin % 60;
    document.getElementById('w-daylight').textContent = `${dlH}h ${dlM}m of daylight`;

    const hourlyHTML = [];
    for (let i = ci; i < Math.min(ci + 24, data.hourly.time.length); i++) {
        const isoT = data.hourly.time[i];
        const h = hoursFromISO(isoT);
        const hIsDay = h >= 6 && h < 20;
        const isNow = i === ci;
        const hTime = isNow ? 'Now' : fmt12(h, 0).replace(':00 ', ' ');
        hourlyHTML.push(`
            <div class="hour-card ${isNow ? 'now' : ''}">
                <div class="h-time">${hTime}</div>
                <div class="h-icon">${getWeatherIcon(data.hourly.weathercode[i], hIsDay)}</div>
                <div class="h-temp">${toUnit(data.hourly.temperature_2m[i])}°</div>
                ${data.hourly.precipitation_probability[i] > 0 ? `<div class="h-precip"><img src="${METEO_CDN}/raindrop.svg" alt="">${data.hourly.precipitation_probability[i]}%</div>` : ''}
            </div>
        `);
    }
    ...
    for (let i = 0; i < data.daily.time.length; i++) {
        ...
        const hi = toUnit(data.daily.temperature_2m_max[i]);
        const lo = toUnit(data.daily.temperature_2m_min[i]);
        ...
        dailyHTML.push(`
            <div class="daily-row">
                ...
                <div class="day-icon">${getWeatherIcon(data.daily.weathercode[i], true)}</div>
...
function handleKeyPress(e) { if (e.key === 'Enter') fetchWeather(); }
document.getElementById('location-input').addEventListener('input', hideError);
showState('welcome');
